...
namespace EDLib {
  namespace gf {
    ...
    public:
      PairingSusceptibility(alps::params &p, Hamiltonian &h, Args ... args) : Lanczos < Hamiltonian, Mesh, Args... >(p, h, args...), _model(h.model()),
        ...
        if(input_file.is_data("PairingSusceptibility_orbitals/values")){
          input_file >> alps::make_pvp("PairingSusceptibility_orbitals/values", gf_orbs);
        }else{
...
//        std::cout<<"Skipped by Boltzmann factor."<<std::endl;
    ...
    private:
      ...
      void local_contribution(const EigenPair<precision, typename Hamiltonian::ModelType::Sector>& pair, const EigenPair<precision, typename Hamiltonian::ModelType::Sector>& groundstate) {
        ...
        for (int iorb = 0; iorb < diagonal_orbs.size(); ++iorb) {
          ...
          for (int ispin = 0; ispin < _model.spins(); ++ispin) {
            ...
            if (create_two_particles(orb, pair.eigenvector(), outvec, expectation_value)) {
            ...
            if (annihilate_two_particles(orb, pair.eigenvector(), outvec, expectation_value)) {
              int nlanc = lanczos(outvec);
#ifdef USE_MPI
              if(!rank)
#endif
      ...
      void nonlocal_contribution(const EigenPair<precision, typename Hamiltonian::ModelType::Sector>& pair, const EigenPair<precision, typename Hamiltonian::ModelType::Sector>& groundstate) {
        ...
        for (int iorb = 0; iorb < offdiagonal_orbs.size(); ++iorb) {
          for (int ispin = 0; ispin < _model.spins(); ++ispin) {
            ...
            for(int i = 0; i < 2; ++i){
              _model.symmetry().set_sector(pair.sector());
              found[i] = create_two_particles(orbs[i], pair.eigenvector(), outvec[i], expectation_value);
            }
            if(found[0] || found[1]){
              if(found[0] && found[1]){
               sumvec = outvec[0];
            ...
            for(int i = 0; i < 2; ++i){
              /// restore symmetry sector
              _model.symmetry().set_sector(pair.sector());
              found[i] = annihilate_two_particles(orbs[i], pair.eigenvector(), outvec[i], expectation_value);
            }
            if(found[0] || found[1]){
              if(found[0] && found[1]){
               sumvec = outvec[0];
       ...
       *
       * @param orbital - the orbital to create the particles, first with the spin down.
       ...
       */
      bool create_two_particles(int orbital, const std::vector < precision > &invec, std::vector < precision > &outvec, double &expectation_value) {
        ...
        hamiltonian().storage().a_adag(orbital + _model.orbitals(), invec, tmpvec, tmp_sec, false);
        ...
        common::statistics.registerEvent("adag");
        hamiltonian().storage().a_adag(orbital, tmpvec, outvec, next_sec, false);
        common::statistics.updateEvent("adag");
        std::cout<<"adag in "<<common::statistics.event("adag").first<<" s \n";
       ...
       *
       * @param orbital - the orbital to destroy the particles, first with the spin down.
       * @param invec - current eigenstate
       * @param outvec - Op-vec product
       * @param expectation_value - expectation value of a*a
       * @return true if the particle has been destroyed
       */
      bool annihilate_two_particles(int orbital, const std::vector < precision > &invec, std::vector < precision > &outvec, double &expectation_value) {
        // check that the particle can be annihilated
        ...
        hamiltonian().storage().a_adag(orbital + _model.orbitals(), invec, tmpvec, tmp_sec, true);
        ...
        common::statistics.registerEvent("a");
        hamiltonian().storage().a_adag(orbital, tmpvec, outvec, next_sec, true);
        common::statistics.updateEvent("a");
        std::cout<<"a in "<<common::statistics.event("a").first<<" s \n";
        double norm = hamiltonian().storage().vv(outvec, outvec);
        for (int j = 0; j < outvec.size(); ++j) {
